...
router.get("/", authenticateJWT, async (req, res) => {
  try {
    ...
    // Get accepted requests (sent by user) that haven't been viewed
    ...
    unreadMessages.forEach((msg) => {
      ...
      if (
        !unreadMessagesByMatch[matchId] ||
        new Date(msg.createdAt) >
          new Date(unreadMessagesByMatch[matchId].createdAt)
      ) {
    ...
    // Add request notifications (pending requests received)
    ...
    // Add request accepted notifications (accepted requests sent by user)
    ...
    Object.values(unreadMessagesByMatch).forEach((message) => {
      const match = matches.find(
        (m) => m._id.toString() === message.match._id.toString()
      );
      if (match) {
        const otherUser =
...
router.post("/:id/read", authenticateJWT, async (req, res) => {
  try {
    ...
    } else if (type === "request" || type === "request_accepted") {
