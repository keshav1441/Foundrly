...
router.get('/google', (req, res) => {
  ...
  if (!clientID || clientID === 'mock') {
    return res.status(400).json({ error: 'Google OAuth not configured. Please set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in .env file.' });
  }
...
router.get('/google/callback', async (req, res) => {
  try {
    ...
    if (!clientID || !clientSecret || clientID === 'mock' || clientSecret === 'mock') {
    ...
    console.log('Google OAuth Callback - Redirecting to frontend:', redirectUrl);
...
router.get('/github', (req, res) => {
  ...
  const redirectURI = `${process.env.BACKEND_URL || 'http://localhost:4000'}/auth/github/callback`;
...
router.get('/github/callback', async (req, res) => {
  try {
    ...
    const email = primaryEmail?.email || userResponse.data.login + '@github.local';
    ...
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
    const redirectUrl = `${frontendUrl}/auth/callback?token=${encodeURIComponent(token)}`;
    console.log('GitHub OAuth Callback - Redirecting to frontend:', redirectUrl);
    res.redirect(redirectUrl);
  } catch (error) {
