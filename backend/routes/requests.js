...
router.post('/', authenticateJWT, async (req, res) => {
  try {
    ...
    if (ideaOwnerId === userId) {
      return res.status(400).json({ error: 'You cannot request your own idea' });
    }
    ...
    if (existingRequest) {
      return res.status(400).json({ error: 'You have already sent a request for this idea' });
    }
...
router.post('/:id/accept', authenticateJWT, async (req, res) => {
  try {
    ...
    if (request.ideaOwner._id.toString() !== userId) {
      return res.status(403).json({ error: 'Only the idea owner can accept requests' });
    }
    ...
    if (ioInstance) {
      try {
        const chatNamespace = ioInstance.of('/chat');
        chatNamespace.to(`user:${request.requester._id}`).emit('match_notification', populatedMatch);
        chatNamespace.to(`user:${request.ideaOwner._id}`).emit('match_notification', populatedMatch);
      } catch (error) {
        console.error('Socket.io emit error:', error);
      }
    }
...
router.post('/:id/reject', authenticateJWT, async (req, res) => {
  try {
    ...
    if (request.ideaOwner.toString() !== userId) {
      return res.status(403).json({ error: 'Only the idea owner can reject requests' });
    }
